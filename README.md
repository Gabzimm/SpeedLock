...
USB e Wi-Fi têm categorias próprias no mesmo ecrã, com ligação real
(porta série/socket abertos de verdade) — ver "Transportes USB e Wi-Fi"
abaixo para o que ainda falta.
...
`usb_transport.dart` e `wifi_transport.dart` implementam ligação real —
abrem mesmo a porta USB-série ou o socket Wi-Fi. O que continua por
confirmar, tal como no BLE, é o codec de comandos de qualquer marca —
isso exige o mesmo trabalho de engenharia reversa com hardware real.
Ligar por USB/Wi-Fi funciona (mostra "ligado"), mas qualquer comando
falha com um erro claro em vez de mandar bytes inventados para a
trotinete.
